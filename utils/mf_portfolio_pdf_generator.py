...
def _fmt_inr(val):
    ...
    s = str(n)
    if len(s) > 3:
        s = s[:-3][-1::-1]  # digits before last 3, reversed
...
def _build_commentary_prompt(portfolio_data: dict) -> str:
    ...
    def _crore(v):
        """Express a rupee value as a crore string, e.g. 10037302 → 'Rs.1.00 crore'."""
        if not v:
            return "N/A"
        c = v / 1_00_00_000
        if c >= 1:
            return f"Rs.{c:.2f} crore"
        l = v / 1_00_000
        return f"Rs.{l:.2f} lakh"

    header = (
        f"Client: {d.get('client_name', 'N/A')} | "
        f"Report Date: {d.get('report_date', 'N/A')}\n"
        f"Investment Start: {d.get('investment_start', 'N/A')} | "
        f"Invested: {_crore(total_inv)} | "
        f"Current Value: {_crore(total_cur)} | "
        f"Gain: {_crore(total_gain)} | "
    ...
    # ── Build reliable equity / hybrid name sets from the passed lists ───────
    # These are already correctly classified by the metrics pipeline and power
    # the portfolio snapshot table — use them as the primary classifier rather
    # than relying on the 'nature' field which may be absent on all_funds rows.
    _equity_names = {f["name"] for f in (d.get("equity_funds") or [])}
    _hybrid_names = {f["name"] for f in (d.get("hybrid_funds") or [])}

    def _fund_heading(fund: dict) -> str:
        name = fund.get("name", "")
        if name in _equity_names:
            return "Your Equity Funds"
        if name in _hybrid_names:
            return "Your Hybrid Funds"
        # Remaining funds: classify by nature then by benchmark index name then by fund name
        nature = str(fund.get("nature", "")).strip().lower()
        if "debt" in nature:
            return "Your Debt Funds"
        bench = str(fund.get("benchmark_index", "")).lower()
        if "debt" in bench or "bond" in bench or "gilt" in bench or "liquid" in bench:
            return "Your Debt Funds"
        # Last resort: infer from fund name keywords (handles stale/missing benchmark_index)
        ...
        if ("debt" in name_lower or "bond" in name_lower or "gilt" in name_lower
            ...
            return "Your Debt Funds"
        return "Your Other Funds"

    # ── Group funds by section heading ────────────────────────────────────────
    from collections import defaultdict
    grouped: dict[str, list] = defaultdict(list)
    for f in funds_source:
        grouped[_fund_heading(f)].append(f)

    # Canonical section order — exactly 4 fund-type sections
    _SECTION_ORDER = [
        "Your Equity Funds",
        "Your Hybrid Funds",
        "Your Debt Funds",
    ...
    present_sections = [s for s in _SECTION_ORDER if s in grouped]
    ...
    # ── Build section-grouped fund block ─────────────────────────────────────
    fund_block_lines: list[str] = []
    for section in present_sections:
        fund_block_lines.append(f"{section}:")
        for f in grouped[section]:
            fund_block_lines.append(_fund_line(f))
        fund_block_lines.append("")
    ...
    # ── Build section heading instructions ────────────────────────────────────
    # Always include all 4 fund-type sections + the fixed headings; skip if no funds in that section
    section_list = "\n".join(f"  {s}" for s in ["How Your Portfolio Has Done Overall"]
                             + present_sections + ["Key Observations"])
    ...
    lines = [
        ...
        "",
        "Use EXACTLY these section headings, each on its own line:",
        section_list,
        "",
        "Write 3-5 sentences per section.",
        "Reference actual fund names and XIRR values.",
        ...
        "When mentioning monetary amounts, always express them in crores or lakhs",
        "(e.g. 'Rs.1.00 crore', 'Rs.45.50 lakh'). Never write raw numbers or use comma-separated digits.",
        "",
        "CRITICAL RULE — new funds (invested < 12 months ago):",
        f"Today's report date is {d.get('report_date', 'N/A')}.",
        "Each fund line shows 'Invested since: <date>'. If a fund was invested less than 12 months",
        "before the report date, do NOT analyse or comment on its performance — simply acknowledge",
        "it as a recent addition and note that meaningful performance assessment requires at least",
        "12 months of investment history.",
        "",
        "CRITICAL RULE — benchmark handling:",
        "When a fund has benchmark data (Benchmark index and Trailing Returns are provided),",
        "reference the benchmark index name and trailing returns, and use the pre-computed Verdict",
        "(OUTPERFORMS / UNDERPERFORMS benchmark) exactly as given — never override it.",
        "When a fund has NO benchmark data (no Benchmark line in the data), do NOT mention the",
        "benchmark at all. Do NOT say 'benchmark not found' or 'benchmark not available'.",
        "Simply discuss the fund's XIRR and what it means for the client.",
        "",
        "--- Portfolio Data ---",
        header,
        "",
...
def _parse_commentary(raw):
    headings = [
        ...
        "Portfolio Overview",
        "Quarter in Review",
        # Fund-category headings — 4 canonical sections
        "Your Equity Funds",
        "Your Hybrid Funds",
        "Your Hybrid Fund",
        "Your Debt Funds",
    ...
    for line in raw.splitlines():
        ...
        if matched:
            if cur_h and cur_b:
                blocks.append({'heading': cur_h, 'body': ' '.join(cur_b).strip()})
            cur_h = matched
            cur_b = []
...
def generate_ai_commentary(portfolio_data):
    ...
    for f in (portfolio_data.get("all_funds") or []):
        ...
        b3yr  = f.get("benchmark_return_3yr")
        ...
        logger.debug(
            ...
            bench,
            f"{b1yr:.2f}%" if b1yr is not None else "None",
            f"{b3yr:.2f}%" if b3yr is not None else "None",
...
class MFPortfolioPDFGenerator:
    ...
    def _section_snapshot(self, d):
        ...
        kpi_t = Table([val_row, lbl_row], colWidths=[1.55*inch, 1.55*inch, 1.55*inch, 1.2*inch, 1.15*inch])
        kpi_ts = TableStyle([
