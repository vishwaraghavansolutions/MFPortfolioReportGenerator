...
def _build_commentary_prompt(portfolio_data: dict) -> str:
    ...
    def _fund_heading(fund: dict) -> str:
        ...
        if "gold" in bench or "silver" in bench:
            return "Your Gold & Silver Funds"
    ...
    def _fund_line(f: dict) -> str:
        ...
        if fx is not None and bench_ref is not None:
            verdict = "OUTPERFORMS benchmark" if fx > bench_ref else "UNDERPERFORMS benchmark"
        else:
            verdict = "benchmark comparison not available"
        return (
            f"  {f.get('name', '—')}: "
            f"Your XIRR={fx_str} | "
    ...
    lines = [
        ...
        "",
        "CRITICAL RULE — performance verdict:",
        "Each fund already has a pre-computed Verdict field (OUTPERFORMS / UNDERPERFORMS benchmark).",
        "You MUST use that verdict exactly as given. Never override it by re-comparing the numbers yourself.",
        "A fund OUTPERFORMS only when its XIRR is strictly greater than the benchmark return.",
        "A fund UNDERPERFORMS when its XIRR is strictly less than the benchmark return.",
        "",
        "--- Portfolio Data ---",
        header,
        "",
...
def _parse_commentary(raw):
    headings = [
        ...
        "Portfolio Overview",
        "Quarter in Review",
        # Fund-category headings (dynamic — must list all possible variants)
        "Your Equity Funds",
        "Your Hybrid Funds",
        "Your Hybrid Fund",
        "Your Debt Funds",
        "Your Debt Fund",
        "Your Gold & Silver Funds",
        "Your Gold & Silver Fund",
        "Your Gold and Silver Funds",
        "Your Other Funds",
        "Your Other Fund",
        # Legacy / fallback
        "Equity Portfolio",
