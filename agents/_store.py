...
class IndexStore:
    ...
    def load(self, parquet_dir: str, force_reload: bool = False) -> Dict[str, Any]:
        ...
        """
        pdir    = Path(parquet_dir).resolve()   # canonical absolute path
        loaded  : List[str] = []
        skipped : List[str] = []
        newly_loaded = False
        ...
        for path in sorted(pdir.glob("*.parquet")):
            ...
            key = str(path.resolve())               # always absolute — cwd-independent
        ...
        if newly_loaded:
            self._combined = None                   # invalidate merged cache only when needed
        return {
            "loaded" : loaded,
            "skipped": skipped,
