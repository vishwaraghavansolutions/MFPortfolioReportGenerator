...
def _playwright_fetch_index(
    ...
    page.wait_for_selector("table tbody tr", timeout=_PW_TIMEOUT)
    page.wait_for_timeout(1_000)
...
def _upload_csv_to_gcs(local_path: Path, filename: str) -> str:
    """Upload a local CSV to GCS winrich_shared/Master/{filename}. Returns gcs_uri."""
...
def skill_download_index_data(params: Dict[str, Any]) -> AgentResponse:
    ...
    if year is None:
        return AgentResponse(status=AgentStatus.FAILED, error="'year' param is required.")
    year = int(year)
    ...
    logger.info(
        ...
        f"for {year} ({_fmt_date(start_date)} → {_fmt_date(end_date)})  "
        f"[headless={headless}]"
    )
    ...
    # Upload to GCS
    try:
        gcs_uri = _upload_csv_to_gcs(local_csv, filename)
