...
def load_data():
    resp = GCSStorageAgent().run("load_ranking_csv", {"filename": "fund_ranking.csv"})
    if resp.status != AgentStatus.SUCCESS:
        st.error(f"Could not load ranking file from GCS: {resp.error}")
        st.stop()
    raw_df = resp.output["dataframe"]
    raw_df.columns = raw_df.columns.str.strip()

    # Detect whether the CSV has 2 leading metadata/weight rows (legacy format).
    # A metadata row has a Scheme Type value that is NOT a real scheme type.
    _known_types = {"Equity Scheme", "Debt Scheme", "Hybrid Scheme"}
    _first_val = str(raw_df.iloc[0].get("Scheme Type", "")).strip() if len(raw_df) else ""
    if _first_val not in _known_types:
        metadata_rows = raw_df.iloc[0:2].copy()
        funds_df = raw_df.iloc[2:].copy()
    else:
        metadata_rows = raw_df.iloc[0:0].copy()  # empty
        funds_df = raw_df.copy()
...
if st_type != "Select" and st_cat:
    ...
    if not base_funds.empty:
        ...
        # UI Display Table (Metadata included if present)
        display_meta = df_metadata.copy()
        if not display_meta.empty:
            display_meta['Rank'] = [""] * len(display_meta)
                ...
                if st.button("🚀 Calculate New Ranks"):
                    ...
                    if not cust_meta.empty:
                        for param, weight in user_weights.items():
                            if param in cust_meta.columns and len(cust_meta) > 1:
                                cust_meta.at[cust_meta.index[1], param] = weight
                        cust_meta['Rank'] = [""] * len(cust_meta)
